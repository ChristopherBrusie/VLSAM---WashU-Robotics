...
if [ "6.0" != "$PATCHES_REV" ] && [ "6.2" != "$PATCHES_REV" ]; then
...
if [ "6.2" = "$PATCHES_REV" ]; then
	TEGRA_SOURCE_SYNC_SH="source_sync.sh"
	TEGRA_TAG="jetson_36.4.3"
fi
...
if [ "6.0" = "$PATCHES_REV" ] || [ "6.2" = "$PATCHES_REV" ]; then # for Jetpack 6
